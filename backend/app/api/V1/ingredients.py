...
from app.schemas.ingredients import Ingredient, IngredientCreate, IngredientUpdate
...
async def update(ingredient_id: str, body: IngredientUpdate, user: dict = Depends(get_current_user)):
    ...
    if not access["canUpdate"]:
        raise HTTPException(status_code=403, detail="Forbidden")
    ok = await update_ingredient(user["restaurantId"], ingredient_id, {k:v for k,v in body.model_dump().items() if v is not None})
    if not ok:
        raise HTTPException(status_code=404, detail="Ingredient not found")
    doc = await get_ingredient(user["restaurantId"], ingredient_id)
    return doc
