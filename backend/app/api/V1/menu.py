...
async def menu_get(forDate: date = Query(...), user: dict = Depends(get_current_user)):
	...
	items = [await populate_menu_item_data(i) for i in (doc.get("items") or [])]
	return Menu(id=str(doc["_id"]), date=doc["date"], items=items)
...
async def menu_create(payload: MenuCreate, user: dict = Depends(get_current_user)):
	...
	if not access.get("canCreate"):
		raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Forbidden")
	enriched = [await populate_menu_item_data(i.model_dump()) for i in payload.items]
	mid = await create_menu(payload.date, enriched)
...
async def menu_update(menu_id: str, payload: MenuUpdate, user: dict = Depends(get_current_user)):
	...
	if not access.get("canUpdate"):
		raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Forbidden")
	enriched = [await populate_menu_item_data(i.model_dump()) for i in payload.items]
	ok = await update_menu(menu_id, enriched)
	if not ok:
		raise HTTPException(status_code=404, detail="Not found")
...
async def menu_range(start: Optional[date] = Query(None), end: Optional[date] = Query(None),
	...
	out = []
	for d in docs:
		items = [await populate_menu_item_data(i) for i in (d.get("items") or [])]
		out.append(Menu(id=str(d["_id"]), date=d["date"], items=items))
	return out
