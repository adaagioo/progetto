...
from app.schemas.recipes import Recipe, RecipeCreate, RecipeUpdate
...
async def update(recipe_id: str, body: RecipeUpdate, user: dict = Depends(get_current_user)):
    ...
    if not access["canUpdate"]:
        raise HTTPException(status_code=403, detail="Forbidden")
    ok = await update_recipe(user["restaurantId"], recipe_id, {k:v for k,v in body.model_dump().items() if v is not None})
    if not ok:
        raise HTTPException(status_code=404, detail="Recipe not found")
